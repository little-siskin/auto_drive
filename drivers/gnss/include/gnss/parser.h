...
template <class T>
inline T* As(MessagePtr message_ptr) { return dynamic_cast<T*>(message_ptr); }

// An abstract class of Parser.
// One should use the create_xxx() functions to create a Parser object.
class Parser {
public:
    // Return a pointer to a NovAtel parser. The caller should take ownership.---返回指向novatel分析器的指针。呼叫方应取得所有权。
    ...
    // Return a pointer to a unicomm parser. The caller should take ownership.
    static Parser* create_unicomm();
