...
const std::string UTM_TARGET = "+proj=utm +zone=52 +ellps=WGS84 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs ";
...
Parser* create_parser(config::Stream::Format format, bool is_base_station = false) {
    //ROS_INFO("create parser format------" << format.data());
    switch (format) {
    ...
    //new add
    case config::Stream::UNICOMM_BINARY:
        return Parser::create_unicomm();
...
void DataParser::check_gnss_status(::apollo::drivers::gnss::Gnss *gnss) {
    ...
    _gnss_status->mutable_header()->set_timestamp_sec(ros::Time::now().toSec());
    //new add
    _gnss_status->set_lon(static_cast<double>(gnss->mutable_position()->lon()));
    _gnss_status->set_lat(static_cast<double>(gnss->mutable_position()->lat()));
    //-------
    _gnss_status_publisher.publish(_gnss_status);
}
...
void DataParser::dispatch_message(Parser::MessageType type, MessagePtr message) {
    std_msgs::String msg_pub;
    ROS_INFO("----Into dispatch_message----");
    switch (type) {
...
void DataParser::publish_odometry_pb_message(const MessagePtr message) {
    ...
    gps_msg->set_raw_heading(-(ins->euler_angles().z()/DEG_TO_RAD_LOCAL-90.0));
    gps_msg->set_raw_pitch(-ins->euler_angles().y()/DEG_TO_RAD_LOCAL);
    gps_msg->set_raw_roll(ins->euler_angles().x()/DEG_TO_RAD_LOCAL);
    ...
    std::cout<<"ins_ori_x="<<q.x()<<"ins_ori_y="<<q.y()<<"ins_ori_z="<<q.z()<<"ins_ori_w="<<q.w()<<std::endl;
    std::cout<<"ins_vel_x="<<ins->linear_velocity().x()<<"ins_vel_y="<<ins->linear_velocity().y()<<"ins_vel_z="<<ins->linear_velocity().z()<<std::endl;
    std::cout<<"ins_avel_x="<<ins->angular_velocity().x()<<"ins_avel_y="<<ins->angular_velocity().y()<<"ins_avel_z="<<ins->angular_velocity().z()<<std::endl;
    ...
    ROS_INFO("_nav_odometry_publisher.publish(gps)");
}
...
void DataParser::publish_corrimu_pb_message(const MessagePtr message) {
    ...
    std::cout<<"imu_accvel_x="<<-ins->linear_acceleration().y()<<" imu_accvel_y="<<ins->linear_acceleration().x()<<" imu_accvel_z="<<ins->linear_acceleration().z()<<std::endl;
    std::cout<<"imu_angvel_x="<<-ins->angular_velocity().y()<<" imu_angvel_y="<<ins->angular_velocity().x()<<" imu_angvel_z="<<ins->angular_velocity().z()<<std::endl;
    ...
    ROS_INFO("_imu_publisher.publish(imu);");
}
