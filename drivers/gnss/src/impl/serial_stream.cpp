...
SerialStream::SerialStream(const char* device_name,
                           speed_t baud_rate,
...
void SerialStream::open(void) {
    ...
    if (fd == -1) {
        ...
        case EMFILE:
        default:
            ROS_ERROR_STREAM_THROTTLE(2, "Open device "
                             << _device_name << " failed, error: " << strerror(errno));
            return ;
        }
    }
...
void SerialStream::check_remove() {
    ...
    if (nsent < 0) {
        ROS_ERROR_STREAM("Serial stream detect write failed, error: "
                          << strerror(errno));
        switch (errno) {
            case EBADF:
            case EIO:
...
size_t SerialStream::read(uint8_t* buffer, size_t max_length) {
    ...
    ROS_INFO("-----enter serial_stream  read-------");

    //wait_readable(100000); //wait 10ms
    //sleep(10);
    wait_readable(500000);
    long counter = 0;
    while (max_length > 0) {
        bytes_current_read = ::read(_fd, buffer, max_length);
        counter+=bytes_current_read;
        ...
        //new add
        //ROS_INFO("bytes_current_read");
        if (bytes_current_read == 0 && counter >= 400) {
            if (!bytes_read) {
                check_remove();
                return 0;
            }
