...
class RtkRecord(object):
    ...
    def __init__(self, record_file):
        ...
        self.write("x,y,z,speed,acceleration,curvature,"\
                        "curvature_change_rate,time,theta,gear,s,throttle,brake,steering,carrawheading\n")
    ...
    def localization_callback(self, data):
        ...
        carrawheading = self.localization.pose.raw_heading
        if math.isnan(self.chassis.speed_mps):
            self.logger.warning("find nan speed_mps: %s" % str(self.chassis))
            #return
        if math.isnan(self.chassis.steering_percentage):
	    ...
	    carspeed = self.localization.pose.linear_velocity.y
	...
	#speed_epsilon = 0;
        ...
        curvature = math.tan(math.radians(carsteer / 100 * 480) / 480) / 2.9
        ...
        if self.startmoving:
            ...
            self.write(
                "%s, %s, %s, %s, %s, %s, %s, %.4f, %s, %s, %s, %s, %s, %s, %s\n" %
